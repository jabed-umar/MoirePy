...
This page loads benchmark JSON files and renders interactive performance plots.
...
(() => {
  const DATASETS = [
    { label: "i5 Gen12", path: "../benchmark/i5_gen12/benchmark_results.json", color: "#1472e6" },
    { label: "i3 Gen4", path: "../benchmark/i3_gen4/benchmark_results.json", color: "#e95d2a" },
    { label: "Raspberry Pi 5", path: "../benchmark/pi5/benchmark_results.json", color: "#0f9d58" },
  ];
  ...
  function renderCharts() {
    ...
    if (enabled.length === 0) {
      if (exp1Plot) { exp1Plot.destroy(); exp1Plot = null; }
      if (exp2Plot) { exp2Plot.destroy(); exp2Plot = null; }
